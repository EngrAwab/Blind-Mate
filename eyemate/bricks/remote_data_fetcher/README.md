...
Your Brick **remote_data_fetcher** is ready!

Custom Bricks are modular components that add reusable functionality to your app.

## How to use it

Write your Python class inside `bricks/remote_data_fetcher/__init__.py` and add the `@brick` decorator.
```python
# bricks/remote_data_fetcher/__init__.py
...
your brick is created under the `bricks/remote_data_fetcher` folder that contains the following files:
...
```yaml
id: remote_data_fetcher
name: remote_data_fetcher
variables:
    - name: YOUR_NAME
